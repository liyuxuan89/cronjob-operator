...
func (r *CronJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	isJobFinished := func(job *v1.Job) (bool, v1.JobConditionType) {
		for _, c := range job.Status.Conditions {
			if (c.Type == v1.JobComplete || c.Type == v1.JobFailed) && c.Status == v12.ConditionTrue {
				return true, c.Type
			}
		}
		return false, ""
	...
	getScheduledTimeForJob := func(job *v1.Job) (*time.Time, error) {
	...
	constructJobForCronJob := func(cronJob *batchv1.CronJob, scheduledTime time.Time) (*v1.Job, error) {
	...
	if err != nil {
		logger.Error(err, "unable to construct job from templates")
...
func (r *CronJobReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	// create index for job owner
	if err := mgr.GetFieldIndexer().IndexField(ctx, &v1.Job{}, jobOwnerKey, func(rawObj client.Object) []string {
